			...
			if (cursor.moveToFirst()) {
				do {
					...
					orderTrans.setiTransactionID(
							cursor.getInt(cursor.getColumnIndex(TransactionEntry.COLUMN_TRANSACTION_ID)));
					...
					orderTrans.setDtOpenTime(Util.dateTimeFormat(
							cursor.getString(cursor.getColumnIndex(TransactionEntry.COLUMN_OPEN_TIME)), "yyyy-MM-dd HH:mm:ss"));
					...
					orderTrans.setiReceiptYear(
							cursor.getInt(cursor.getColumnIndex(TransactionEntry.COLUMN_RECEIPT_YEAR)));
					orderTrans.setiReceiptMonth(
							cursor.getInt(cursor.getColumnIndex(TransactionEntry.COLUMN_RECEIPT_MONTH)));
					orderTrans.setiReceiptID(
							cursor.getInt(cursor.getColumnIndex(TransactionEntry.COLUMN_RECEIPT_ID)));
					orderTrans.setSzReceiptNo(
							cursor.getString(cursor.getColumnIndex(TransactionEntry.COLUMN_RECEIPT_NO)));
					orderTrans.setDtSaleDate(Util.dateTimeFormat(
							cursor.getString(cursor.getColumnIndex(TransactionEntry.COLUMN_SALE_DATE)),"yyyy-MM-dd"));
					orderTrans.setfTransVAT(
							MPOSApplication.fixesDigitLength(4, cursor.getDouble(cursor.getColumnIndex(TransactionEntry.COLUMN_TRANS_VAT))));
					orderTrans.setfTransactionVatable(
							MPOSApplication.fixesDigitLength(4, cursor.getDouble(cursor.getColumnIndex(TransactionEntry.COLUMN_TRANS_VATABLE))));
					orderTrans.setiSessionID(
							cursor.getInt(cursor.getColumnIndex(SessionEntry.COLUMN_SESS_ID)));
					orderTrans.setiVoidStaffID(cursor.getInt(cursor
							.getColumnIndex(TransactionEntry.COLUMN_VOID_STAFF_ID)));
					orderTrans.setSzVoidReason(
							cursor.getString(cursor.getColumnIndex(TransactionEntry.COLUMN_VOID_REASON)));
					orderTrans.setDtVoidTime(Util.dateTimeFormat(
							cursor.getString(cursor.getColumnIndex(TransactionEntry.COLUMN_VOID_TIME)), "yyyy-MM-dd HH:mm:ss"));
					...
					orderTrans.setfTransactionExcludeVAT(MPOSApplication.fixesDigitLength(4,cursor.getDouble(
							cursor.getColumnIndex(TransactionEntry.COLUMN_TRANS_EXCLUDE_VAT))));
			...
			if(cursor.moveToFirst()){
				do{
					...
					if(cursor.getDouble(cursor
							.getColumnIndex(OrderDetailEntry.COLUMN_PRICE_DISCOUNT)) > 0){

						SaleTable_OrderPromotion promotion = new SaleTable_OrderPromotion();
						promotion.setiTransactionID(cursor.getInt(cursor
							.getColumnIndex(TransactionEntry.COLUMN_TRANSACTION_ID)));
						promotion.setiComputerID(cursor.getInt(cursor
							.getColumnIndex(ComputerEntry.COLUMN_COMPUTER_ID)));
						promotion.setiOrderDetailID(cursor.getInt(cursor
							.getColumnIndex(OrderDetailEntry.COLUMN_ORDER_ID)));
						promotion.setfDiscountPrice(
								MPOSApplication.fixesDigitLength(4, cursor.getDouble(cursor
									.getColumnIndex(OrderDetailEntry.COLUMN_PRICE_DISCOUNT))));
						promotion.setfPriceAfterDiscount(
								MPOSApplication.fixesDigitLength(4, cursor.getDouble(cursor
									.getColumnIndex(OrderDetailEntry.COLUMN_TOTAL_SALE_PRICE))));
		...
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				do {
					...
					order.setfSalePrice(
							MPOSApplication.fixesDigitLength(4, cursor.getDouble(
									cursor.getColumnIndex(OrderDetailEntry.COLUMN_TOTAL_SALE_PRICE))));
					order.setfTotalVatAmount(
							MPOSApplication.fixesDigitLength(4, cursor.getDouble(
									cursor.getColumnIndex(OrderDetailEntry.COLUMN_TOTAL_VAT))));
					...
					saleSessEnd
							.setDtEndDayDateTime(Util.dateTimeFormat(
									cursor.getString(cursor
											.getColumnIndex(SessionEntry.COLUMN_ENDDAY_DATE)),
									"yyyy-MM-dd HH:mm:ss"));
					saleSessEnd
							.setfTotalAmountReceipt(MPOSApplication.fixesDigitLength(
									4,
									cursor.getDouble(cursor
											.getColumnIndex(SessionEntry.COLUMN_TOTAL_AMOUNT_RECEIPT))));
					saleSessEnd.setiTotalQtyReceipt(cursor.getInt(cursor
							.getColumnIndex(SessionEntry.COLUMN_TOTAL_QTY_RECEIPT)));
				} while (cursor.moveToNext());
			} else {
			...
			if (cursor.moveToFirst()) {
				do {
					saleSess.setiSessionID(cursor.getInt(cursor
							.getColumnIndex(SessionEntry.COLUMN_SESS_ID)));
					...
					saleSess.setDtOpenSessionDateTime(Util.dateTimeFormat(
							cursor.getString(cursor
									.getColumnIndex(SessionEntry.COLUMN_OPEN_DATE)),
							"yyyy-MM-dd HH:mm:ss"));
					saleSess.setDtCloseSessionDateTime(Util.dateTimeFormat(
							cursor.getString(cursor
									.getColumnIndex(SessionEntry.COLUMN_CLOSE_DATE)),
							"yyyy-MM-dd HH:mm:ss"));
					...
					saleSess.setfOpenSessionAmount(MPOSApplication.fixesDigitLength(
							4,
							cursor.getDouble(cursor
									.getColumnIndex(SessionEntry.COLUMN_OPEN_AMOUNT))));
					saleSess.setfCloseSessionAmount(MPOSApplication.fixesDigitLength(
							4,
							cursor.getDouble(cursor
									.getColumnIndex(SessionEntry.COLUMN_CLOSE_AMOUNT))));
					saleSess.setiIsEndDaySession(cursor.getInt(cursor
							.getColumnIndex(SessionEntry.COLUMN_IS_ENDDAY)));
				} while (cursor.moveToNext());
			}
			cursor.close();
		}
