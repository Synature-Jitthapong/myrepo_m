	...
	public MPOSOrderTransaction.MPOSOrderDetail getSummaryOrderForDiscount(
		...
		Cursor cursor = getReadableDatabase().rawQuery(
				"SELECT SUM (" + OrderDetailTable.COLUMN_ORDER_QTY + ") "
						...
						+ "(SELECT SUM(" + ProductsTable.COLUMN_PRODUCT_PRICE + "*" + OrderSetTable.COLUMN_ORDER_SET_QTY + ")"
						+ " FROM " + OrderSetTable.TABLE_ORDER_SET
						+ " WHERE " + OrderTransactionTable.COLUMN_TRANSACTION_ID + "=?) AS TotalSetPrice,"
						+ "( SELECT SUM(" + ProductsTable.COLUMN_PRODUCT_PRICE + "*" + OrderDetailTable.COLUMN_ORDER_QTY + ")"
	...
	public List<MPOSOrderTransaction.MPOSOrderDetail> listAllOrderForDiscount(
		...
		Cursor cursor = getReadableDatabase().rawQuery(
				" SELECT a." + OrderTransactionTable.COLUMN_TRANSACTION_ID + ", "
						...
						+ " a." + ProductsTable.COLUMN_PRODUCT_TYPE_ID + ", "
...
//		}
				...
				if (detailCursor.moveToFirst()) {
					do {
						...
						detail.setProductId(detailCursor.getInt(
								detailCursor.getColumnIndex(ProductsTable.COLUMN_PRODUCT_ID)));
						detail.setProductName(detailCursor.getString(
								detailCursor.getColumnIndex(ProductsTable.COLUMN_PRODUCT_NAME)));
		...
		if(cursor.moveToFirst()){
			do{
				...
				comment.setCommentQty(cursor.getDouble(
						cursor.getColumnIndex(OrderDetailTable.COLUMN_ORDER_QTY)));
	...
	public static class OrderCommentTable{
		...
		private static final String SQL_CREATE = "CREATE TABLE " + TABLE_ORDER_COMMENT + " ( "
				...
				+ OrderDetailTable.COLUMN_ORDER_QTY + " REAL NOT NULL DEFAULT 0, "
				+ ProductsTable.COLUMN_PRODUCT_PRICE + " REAL NOT NULL DEFAULT 0 );";
